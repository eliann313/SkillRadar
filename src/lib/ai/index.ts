...
export class AIService {
    ...
    static async generateStructuredObject<T extends object>(options: AIServiceOptions): Promise<T> {
        ...
        const primaryModel = options.userSettings?.preferredModel || "gemini-2.5-flash";
        ...
        const finalErrMessage = lastError instanceof Error ? lastError.message : String(lastError);
        throw new Error(`[AIService] Todos los proveedores e inferencias de fallback fallaron: ${finalErrMessage}`);
    }
}
