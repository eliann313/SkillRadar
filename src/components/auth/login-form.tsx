  ...
  const handleCredentialsAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        toast.success(
          "¡Cuenta creada con éxito! Iniciando sesión automáticamente...",
        );
        ...
        if (loginRes?.error) {
          toast.error(
            "Error al iniciar sesión tras el registro. Por favor ingresa manualmente.",
          );
        ...
        if (loginRes?.error) {
          ...
          if (
            loginRes.error.includes("RATE_LIMIT_EXCEEDED") ||
            loginRes.error.includes("Configuration")
          ) {
            toast.error(
              "Has alcanzado el límite de intentos de login. Espera 15 minutos.",
            );
          } else {
            toast.error(
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm shadow-xl transition-all duration-300">
          ...
          <CardContent className="flex flex-col gap-5">
            ...
            <form
              onSubmit={(e) => void handleCredentialsAuth(e)}
